-- ============================================
-- @set9p | SILENT AIM (360°) + AUTO TRIGGER (CLASSIC) + ESP + ANTI-AIM (DOWN)
-- ============================================
...
local Settings = {
    ...
    -- Анти-аим (сгиб в ноги в 3-м лице)
    AntiAimDown = false,
...
-- Проверка прицела с использованием той же функции isVisible и текущего хитбокса
        ...
        if model then
            ...
            if plr and isEnemy(plr) then
                ...
                if char then
                    local targetHitPart = nil
                    if Settings.HitPart == "Head" then
                        targetHitPart = char:FindFirstChild("Head")
                    else
                        targetHitPart = char:FindFirstChild("UpperTorso") or char:FindFirstChild("Torso")
                    end
                    
                    if targetHitPart and (hitPart == targetHitPart or hitPart.Parent == targetHitPart) then
                        -- Проверяем через ту же общую функцию видимости, что и аим
                        if isVisible(targetHitPart) then
                            return true
                        end
                    end
                end
...
RunService.RenderStepped:Connect(function()
    ...
    -- Моментальный спам-триггер с проверкой видимости аима
    if Settings.AutoTrigger then
        if getTargetUnderCrosshair() then
            task.spawn(function()
                pcall(function()
                    mouse1press()
                    task.wait()
        ...
        if zoomEnabled and Settings.AntiAimDown then
            if torso then
                ...
                if waist and waist:IsA("Motor6D") then
                    waist.C0 = CFrame.new(0, 0, 0) * CFrame.Angles(math.rad(150), 0, 0)
                end
            end
            if head then
...
CombatTab:CreateToggle({
    Name = "Auto Trigger (Моментальный спам)",
    CurrentValue = false,
...
MiscTab:CreateToggle({
    Name = "Анти-Аим (Сгиб в ноги в 3-м лице)",
    CurrentValue = false,
...
Rayfield:Notify({
    Title = "@set9p",
    Content = "Триггер теперь использует единую проверку видимости аима!",
    Duration = 3,
    Image = 4483362458,
})
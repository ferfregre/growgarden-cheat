...
local Settings = {
    ...
    -- Триггербот (Автофаер по целям в FOV)
    TriggerBot = false,
    TriggerDelay = 0,
...
-- ============================
-- ТРИГГЕРБОТ (В РАМКАХ FOV В ПАРЕ С АИМОМ)
-- ============================
task.spawn(function()
    while true do
        task.wait(0.01)
                ...
                if isEnemy(player) then
                    ...
                    if char then
                        ...
                        if hum and hum.Health > 0 then
                            local hitPart = nil
                            if Settings.AimPart == "Torso" then
                                hitPart = char:FindFirstChild("UpperTorso") or char:FindFirstChild("Torso") or char:FindFirstChild("HumanoidRootPart")
                            else
...
Rayfield:Notify({
    Title = "@set9p",
    Content = "Триггербот успешно связан с FOV!",
    Duration = 3,
    Image = 4483362458,
})
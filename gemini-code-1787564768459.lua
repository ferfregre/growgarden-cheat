-- ============================================
-- @set9p | SILENT AIM (360°) + AUTO TRIGGER (INSTANT) + VISIBLE CHECK + ESP + ANTI-AIM (DOWN)
-- ============================================
...
local function GetClosestPlayer()
    ...
    -- Удержание текущей цели, пока она жива и в поле зрения (чтобы не скакала при появлении второго игрока)
    if target and target.Parent then
        ...
        if hum and hum.Health > 0 and hrp then
            local passesVisible = not Settings.VisibleCheck or isVisible(target)
            if passesVisible then
                if Settings.Aim360 then
                    ...
                    if onScreen then
                        ...
                        if distance <= Settings.AimFOV * 1.3 then
                            return target
                        end
                    end
                end
...
-- Мгновенный триггербот без задержек (кулдаунов)
...
CombatTab:CreateToggle({
    Name = "Auto Trigger (Мгновенный спам)",
    CurrentValue = false,
...
Rayfield:Notify({
    Title = "@set9p",
    Content = "Скрипт полностью готов (без задержек)!",
    Duration = 3,
    Image = 4483362458,
})
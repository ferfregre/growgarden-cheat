...
-- DDD (RAY HOOK) + AUTOFIRE + TRIGGERBOT + ESP + BHOP + SPIN + ZOOM + FOV SLIDER + WALLCAM
-- ============================================
...
-- Проверка поддержки hookfunction для Silent Aim
...
local savedCFrame = nil -- Для возврата персонажа на землю
...
-- Постоянное обновление цели для Silent Aim
...
if CanHook then
        ...
        oldRay = hookfunction(Ray.new, function(origin, direction)
            ...
            if dddEnabled and target and target:IsA("BasePart") then
                if trace:find("Client") and not trace:find("10420") and not trace:find("10595") then
                    -- Нормализуем направление луча, сохраняя оригинальную длину
    ...
    if not raySuccess then
        warn("DDD error: " .. tostring(rayError))
    end
else
    warn("Внимание: hookfunction не поддерживается. Меню запущено без функции Silent Aim.")
end
...
local function createFOVCircle()
    ...
    -- Исправлена проверка Drawing API
...
local function toggleWallCam()
    ...
    if wallCamEnabled then
        ...
        -- Сохраняем CFrame перед телепортом
        local char = LocalPlayer.Character
...
Rayfield:Notify({
    Title = "Готово",
    Content = "Скрипт успешно запущен. Проверка функций завершена.",
    Duration = 3,
    Image = 4483362458,
})